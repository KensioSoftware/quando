...
describe("reading a rule from a line of terms", () => {
  ...
  const read = (line: string): string =>
    JSON.stringify(parseRuleExpression(line));
  ...
  describe("terms add up", () => {
    it("narrows once for every term, whatever order they are in", () => {
      ...
      assertTrue(
        sameDefinition(
          ...
          parseRuleExpression("09:00-17:00 mon-fri"),
        ),
      );
    });
    ...
    it("means what the builders mean", () => {
      ...
      const built = all(weekdays(), timeOfDayRange("09:00", "17:00"));
      assertTrue(
        sameDefinition(parseRuleExpression("mon-fri 09:00-17:00"), built),
      );
    });
    ...
    it("ignores the space between terms", () => {
      ...
      assertTrue(
        sameDefinition(
          parseRuleExpression("  weekdays\t09:00-17:00 "),
          parseRuleExpression("weekdays 09:00-17:00"),
        ),
      );
    });
  ...
  describe("a line composes with the builders", () => {
    it("carries the fluent methods a built rule has", () => {
      ...
      const week = parseRuleExpression("mon-fri 09:00-17:00").or(
        parseRuleExpression("sat 10:00-14:00"),
      );
      ...
      assertTrue(isActiveAt(week, when("2026-03-14T12:00")));
      assertTrue(isActiveAt(week, when("2026-03-10T12:00")));
      assertFalse(isActiveAt(week, when("2026-03-14T16:00")));
    });
  });
  ...
  describe("a comma unions inside one term", () => {
    ...
    it("names two windows in a day", () => {
      ...
      const opening = parseRuleExpression("09:00-12:00,14:00-17:00");
      ...
      const built = any(
        ...
        timeOfDayRange("14:00", "17:00"),
      );
      assertTrue(sameDefinition(opening, built));
    });
  ...
  describe("the forms a term can be written in", () => {
    it("takes a weekday in full, short, or shouted", () => {
      ...
      for (const written of ["monday", "mon", "MON", "Monday"]) {
        assertTrue(sameDefinition(parseRuleExpression(written), monday));
      }
    });
    ...
    it("takes an hour without its leading zero", () => {
      ...
      assertTrue(
        sameDefinition(
          parseRuleExpression("9:00-17:00"),
          timeOfDayRange("09:00", "17:00"),
        ),
      );
    });
    ...
    it("takes months by name", () => {
      ...
      assertTrue(sameDefinition(parseRuleExpression("jun-aug"), built));
    });
    ...
    it("takes particular dates", () => {
      ...
      assertTrue(
        sameDefinition(parseRuleExpression("2026-06-15,2026-12-25"), built),
      );
    });
  ...
  describe("qualifiers say what a shape cannot", () => {
    ...
    it("names months by number, for a line a program wrote", () => {
      ...
      assertTrue(sameDefinition(parseRuleExpression("month:3,6"), built));
    });
    ...
    it("excludes whatever term follows it", () => {
      ...
      const rule = parseRuleExpression("weekdays except:2026-12-25");

      // When Christmas and the day after are asked about.
      // Then the exception applies. Both are weekdays in 2026.
      assertFalse(isActiveAt(rule, when("2026-12-25T10:00")));
      assertTrue(isActiveAt(rule, when("2026-12-24T10:00")));
    });
  });
  ...
  describe("the scope the whole line is read in", () => {
    it("reads the line in a named zone", () => {
      ...
      const rule = parseRuleExpression("mon-fri 09:00-17:00 @Europe/London");
      ...
      assertFalse(isActiveAt(rule, when("2026-03-10T10:00", "Asia/Tokyo")));
      assertTrue(isActiveAt(rule, when("2026-03-10T18:00", "Asia/Tokyo")));
    });
    ...
    it("refuses a line that names two zones", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("weekdays @Europe/London @Asia/Tokyo"),
      );
  ...
  describe("what it refuses, and what it says", () => {
    it("refuses a term it cannot read rather than passing over it", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("weekdays sometimes"),
      );
    ...
    it("says where in the line the term was", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("weekdays 09:00-17:00 sometimes"),
      );
    ...
    it("suggests the word a misspelt one was near", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("weekdys"));
    ...
    it("refuses a broken time range rather than reading it as a rule", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("09:00-half"));
    ...
    it("refuses a line that names two calendars", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("M01 cal:hebrew cal:islamic"),
      );
    ...
    it("refuses a range whose far end is not a day", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("mon-someday"));
    ...
    it("refuses a month code that is not one, whatever it ends in", () => {
      ...
      for (const line of ["X01", "Z05L", "101"]) {
        const error = assertThrowsError(() => parseRuleExpression(line));
      ...
      assertTrue(sameDefinition(parseRuleExpression("m01"), monthCodes("M01")));
    });

    it("refuses a stretch with a third date in it", () => {
      // Given two `..` in one term.
      const error = assertThrowsError(() =>
        parseRuleExpression("2026-01-01..2026-06-01..2026-12-31"),
      );
    ...
    it("refuses a cycle with a second anchor", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("every:2w@2026-01-05@2027-01-01"),
      );
    ...
    it("refuses a range with more than two ends", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("mon-wed-fri"));
    ...
    it("refuses a cycle counted in nothing", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("every:2@2026-01-05"),
      );
    ...
    it("refuses a month outside the year", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("month:13"));
    ...
    it("refuses a count that is not a whole number", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("nth:first,monday"),
      );
    ...
    it("refuses a place in the month with no weekday after it", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("nth:1,someday"),
      );
    ...
    it("refuses a day of the month that is not a number", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("day:first"));
    ...
    it("refuses an exception that excludes a scope rather than a rule", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("weekdays except:@Europe/London"),
      );
    ...
    it("refuses a bare number, which could be either of two things", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("15"));
    ...
    it("refuses a cycle with no anchor, and says what one looks like", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("every:2w"));
    ...
    it("refuses `m` as a period, because it reads two ways", () => {
      ...
      const error = assertThrowsError(() =>
        parseRuleExpression("every:2m@2026-01-05"),
      );
    ...
    it("refuses an empty line", () => {
      ...
      const error = assertThrowsError(() => parseRuleExpression("   "));
  ...
  describe("a line of nothing but scope", () => {
    it("covers all of time, on the clock it names", () => {
      ...
      const rule = parseRuleExpression("@Europe/London");
      ...
      assertTrue(isActiveAt(rule, when("2026-03-10T03:00")));
    });
  });
