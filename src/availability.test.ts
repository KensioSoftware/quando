...
import { all, any, dates, timeOfDayRange } from "./build.js";
...
describe("finding available time", () => {
  ...
  const twoOpenings = () =>
    any(
      all(dates("2026-03-09"), timeOfDayRange("09:00", "10:00")),
      all(dates("2026-03-10"), timeOfDayRange("09:00", "12:00")),
    );
  ...
  describe("firstGap", () => {
    ...
    it("uses an explicit horizon without reporting a safety failure", () => {
      ...
      const found = firstAvailableSlot({ type: "never" }, minutes(30), {
        ...inWindow("2026-03-09T00:00"),
        within: { days: 1 },
      });
  ...
  describe("slots", () => {
    ...
    it("stays lazy over a large finite window", () => {
      // Given availability covering the next hundred years.
      const candidates = availableSlots(
        { type: "always" },
        inWindow("2026-03-09T09:00", "2126-03-09T09:00"),
        { every: minutes(15), lasting: minutes(30) },
      );
